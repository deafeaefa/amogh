...
**For quantization, nobody has looked.** The VLM PTQ literature cited above evaluates exclusively on VQA/OCR/chart/doc/reasoning. MBQ frames quantization sensitivity as a *modality* issue (vision vs. language tokens), not a *capability* issue. The only datapoint we found is anecdotal: an applied vehicle-damage paper (2608.02470) observed that a 4-bit GPTQ Qwen-VL 2B classifies damage correctly at 87.3% but "fails consistently" at localizing it. Separately, an empirical Qwen3 quantization study (2505.02214) shows Qwen3 LLMs degrade more than Llama-3 at ≤3 bits and that the smallest models suffer most — exactly the regime edge deployment lives in.
...
Every choice a compression pipeline normally makes by perplexity or VQA — calibration set, group size, loss weights, checkpoint — is made here by D_dev grounding metrics: referring-expression comprehension (REC) accuracy@IoU 0.5 plus mean generalized IoU (GIoU) against ground-truth boxes. General capability is tracked as a *constraint* — VQA and hallucination scores must stay within 1–2 points of the quantized baseline, measured on a fixed VQAv2 subset (~5k, sized so the constraint exceeds sampling noise) and POPE — rather than as the objective. This inversion is itself one of the paper's claims: the field's selection signal has been blind to detection, and any group compressing a perception-oriented VLM can adopt this protocol without adopting anything else in the paper.

### 4.3 Intervention 1: grounding-calibrated PTQ

Run standard GPTQ/AWQ unchanged, but replace the text-corpus calibration set with 256–512 samples from D_probe. Hypothesis: calibration statistics computed on grounding-style activations (image tokens + coordinate emission) preserve the activation ranges that matter for localization better than caption/text calibration does. This is a one-line change to existing pipelines and is evaluated as its own ablation — if it alone recovers a chunk of grounding, that is the simplest possible result in the paper.

### 4.4 Intervention 2: GCL recovery

After quantization, run a short parameter-efficient recovery fine-tune: QLoRA (r=16, α=32) on the frozen quantized base — LoRA is attached to the released GPTQ/AWQ checkpoint itself (via PEFT), so the adapters train against exactly the base they ship on. Teacher-forced on ground-truth answers, the loss is
...
**Benchmarks.** Grounding (primary): RefCOCO/+/g REC accuracy@0.5 (val/testA/testB, with val excluding D_dev; parse JSON, map [0,1000] to pixels) and mean GIoU; a held-out ODinW-13 (Object Detection in the Wild) zero-shot subset as out-of-distribution grounding, never touched by training, calibration, or selection. General (constraint): VQAv2 subset + POPE. Headline quantity: retention ratio **R = REC_quantized / REC_BF16, defined on REC accuracy only** — GIoU is reported as absolute values and deltas, since a ratio of a metric that can sit near zero or go negative is not meaningful.
...
**Baselines and matched controls.** BF16; RTN/GPTQ/AWQ with standard text calibration (the field's status quo); grounding-calibrated PTQ alone (isolates Intervention 1); plain-CE QLoRA recovery without distillation at *matched compute*; unweighted-KL distillation (γ = 1) at *matched steps*; full GCL. The matched-compute and matched-step controls ensure any GCL gain is attributable to coordinate weighting, not to extra training.
...
- **H1 (measurement).** At W4 — and clearly at W3 and W4A8 — grounding retention trails VQA retention by a meaningful margin (target: >10 retention points). Support: the pruning asymmetry (2602.02951, 2412.13180), the 4-bit localization-failure anecdote (2608.02470), Qwen3's documented low-bit fragility (2505.02214).
- **H2 (method).** At the most aggressive setting where H1's gap manifests (expected W4A16; otherwise W3A16 or W4A8), grounding-calibrated PTQ + GCL recovers ≥90% of the BF16 grounding lost by the standard text-calibrated baseline, within 1–2 VQA points of that baseline.
