...
\end{equation}
where $C$ is the set of coordinate-token positions in the teacher-forced target (the tokens covering digits, commas, and brackets inside \bbox{} arrays; well-defined under multi-digit tokenizer merges). This measures exactly the marginal utility the greedy allocator consumes, at the deployment operating point. Because token-level agreement correlates imperfectly with geometric error~\cite{groundingtokens}, the proxy is validated by rank-correlating $s_m$ against decoded REC gains on a module subset before use.
