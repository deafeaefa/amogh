...
for i in $(seq 1 96); do   # up to 8 hours
  sleep 300
  RUNNING=$(pgrep -u eric1 -f "eval_(rec|vqa)" | wc -l)
  ...
  SIZE=$(du -cb $GCQ_RUNS/*.jsonl 2>/dev/null | tail -1 | cut -f1)
  ...
  if [ "$STALLS" -ge 2 ]; then
    ...
    echo "--- processes ---"; ps -o pid,pcpu,etime,cmd -u eric1 | grep -E "eval_(rec|vqa)" | grep -v grep
